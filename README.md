...
В начале метода написать (и удалить после установки Битрикс, иначе не зайдете в админку):
...
- Скрипт установки Битрикс располагается по адресу: https://www.1c-bitrix.ru/download/cms.php
  ...
  - На строке 114, в начале метода `getStringByCharsets($length, $charsetList){ }` временно разместите код (см выше), после установки Битрикс удалите этот код, иначе не зайдете в админку
...
Настройка продукта:
- Сайты > Шаблоны сайтов (можно создать несколько шаблонов).
